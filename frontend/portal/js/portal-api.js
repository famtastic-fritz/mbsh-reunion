(function () {
  ...
  const apiAdapter = {
    ...
    staffPeople: query => request(`staff/people.php?q=${encodeURIComponent(query||'')}`), staffDinner: () => request('staff/dinner.php'), staffInbox: () => request('staff/inbox.php'),
  ...
  const demoFixture = {authenticated:true,csrf_token:'explicit-demo-only',account:{public_id:'demo',email:'demo@example.test',status:'active',email_verified_at:'2026-08-16',first_name:'Demo',last_name:'Hi-Tide',phone:'',city_state:'Miami, FL',graduation_year:1996,display_in_directory:0},staff:{authorized:true,role:'site_owner',label:'Site Owner',capabilities:['view_roster','view_menu','respond_harry','moderate_media','view_inbox','manage_tickets','manage_event_content']}};
  const demoAdapter = {
    ...
    staffDashboard:async()=>({summary:{production_rsvps:10,production_menu_selections:7,pending_reviews:2,unanswered_messages:3,historical_surveys:4,active_attendees:8,operations_attention:1},data_context:{label:'Local demonstration data'}}),staffReviewQueue:async()=>({items:[]}),staffPeople:async()=>({people:[]}),staffDinner:async()=>({summary:{total:7,linked_accounts:6,dietary_notes:2},selections:[]}),staffInbox:async()=>({threads:[]}),staffCommunications:async()=>({delivery:{queued:1,sent:7,failed:0,retried:1}}),staffOperations:async()=>({summary:{pending_orders:1,active_tickets:7,checked_in:0,exceptions:0},orders:[{order_code:'MBSH-DEMO01',contact_name:'Demo Hi-Tide',email:'demo@example.com',quantity:2,total_amount:382,payment_status:'pending',created_at:'2026-09-09 12:00:00'}],tickets:[]}),staffAction:async()=>({ok:true}),staffRecord:async()=>({record:{}}),harryQuestions:async()=>({questions:[]}),harryRespond:async()=>({ok:true}),ownerMemberships:async()=>({memberships:[]}),ownerUpdateMembership:async()=>({ok:true})
  };
