(function () {
  ...
  const routes = [
    ...
    [/stripe|bluevine|payment|processor|webhook/, ['tickets', 'Open Tickets & check-in. A captured request remains pending until the payment processor confirms it. The temporary Shay setup room routes only general setup status to Fritz; never send credentials or financial identifiers through a message.']],
    ...
    [/email|welcome|campaign|delivery|worker|failed|retry|unsubscribe/, ['delivery', 'Site Owners can open Email & workers to inspect accepted, delivered, retried, failed, suppressed, and duplicate-protected campaign states. A provider acceptance is not proof of inbox delivery.']],
