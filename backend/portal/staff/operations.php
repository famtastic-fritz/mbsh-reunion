...
$staff=fam_require_portal_staff($pdo,'manage_tickets');
$snapshot=fam_production_snapshot($config);
$summary=[
  'pending_orders'=>(int)$pdo->query("SELECT COUNT(*) FROM ticket_orders WHERE payment_status='pending'")->fetchColumn(),
...
$orders=$pdo->query("SELECT order_code,contact_name,email,quantity,unit_price,total_amount,price_tier,payment_status,payment_provider,payment_reference,payment_amount,payment_confirmed_at,created_at FROM ticket_orders ORDER BY created_at DESC LIMIT 100")->fetchAll();
fam_json_response(200,['staff'=>fam_portal_staff_client_access($staff),'data_context'=>fam_snapshot_context($snapshot),'summary'=>$summary,'orders'=>$orders,'tickets'=>$tickets]);
